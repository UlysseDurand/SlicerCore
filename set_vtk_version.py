from common import SLICER_DIR, VTK_WHEEL, VTK_SDK_WHEEL
...
def patch_version(pyproject: Path, name: str):
    """Replace occurence of {name}==X.Y.Z with {name}=={version} in file {pyproject}"""
    new_content = ""
    with open(pyproject, "r") as file:
        new_content = re.sub(f"{name}=={VERSION_PATTERN}", f"{name} @ file://{paths[name]}", file.read())
